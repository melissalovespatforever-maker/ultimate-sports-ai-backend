// ============================================
// COACHES TABLE INITIALIZATION - GET VERSION
// Just visit this URL to create tables
// ============================================
...
router.get('/', async (req, res) => {
    try {
        ...
        console.log('🔧 [GET] Initializing coaches tables...');
        ...
        if (checkResult.rows.length === 3) {
            const counts = await pool.query(`
                SELECT 
                    ...
                    (SELECT COUNT(*) FROM coach_stats) as stats,
                    (SELECT COUNT(*) FROM coach_picks) as picks
            `);
        ...
        console.log('📊 Creating coaches tables...');
...
CREATE INDEX idx_picks_sport ON coach_picks(sport);
        ...
        res.status(500).json({
            ...
            error: error.message,
            code: error.code,
            hint: 'Check Railway logs for full error'
        });
    }
});
