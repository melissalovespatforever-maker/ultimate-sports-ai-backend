...
function setupCompetitionsWebSocket(io) {
    ...
    setInterval(async () => {
        try {
            ...
            // Silently ignore leaderboard errors - tables may not exist yet
            if (process.env.NODE_ENV === 'development') {
                console.debug('ℹ️  Leaderboard broadcast skipped (tables may not exist yet)');
            }
        }
    }, 10000);
...
async function getTopLeaderboard() {
    try {
        ...
        // Silently fail if tables don't exist yet (competitions not initialized)
        // Error code 42P01 = relation/table does not exist
        if (error.code === '42P01' || (error.message && error.message.includes('does not exist'))) {
            return [];
        }
        // Silently ignore all leaderboard errors - tables may not be initialized
        return [];
    }
}
